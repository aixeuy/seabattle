...
public class Country {
		...
		case 1:
			if(j==4){
				...
				int k = sc.nextInt();
					...
					switch(p)
					{
					case 1:s.continuation+=ver(s.continuation*0.2);s.cost+=s.weight/4;break;
					case 2:break;
					case 3:s.continuation-=ver(s.continuation*0.2);s.cost-=s.weight/4;break;
					default:break;
					}
	...
	public void addtorepair(){
		if(fleet.numofship!=0){
		Ship[] sh=new Ship[fleet.numofship];
		int n=0;
		for(int i=0;i<fleet.numofship;i++){
			if(fleet.ships[i].health!=fleet.ships[i].resthealth){
				sh[n]=fleet.ships[i];
				n++;
				cash-=(fleet.ships[i].health-fleet.ships[i].resthealth)/2;
			}
		}
		for(int i=0;i<n;i++){
			fleet.delete(sh[i]);
...
//cash-=(repair.ships[i].health-repair.ships[i].resthealth)/2;
	...
	public void arrangebattle(){
		if(fleet.numofship!=0||repair.numofship!=0)
		{
		Scanner sc = new Scanner(System.in);
		...
		System.out.println(fleet.numofship+"choose ship under repair"+"\n"+
		(fleet.numofship+1)+"cancel"); 
        int i = sc.nextInt();
        if(i<fleet.numofship){
        	addtobattle(fleet.ships[i]);
        	fleet.delete(fleet.ships[i]);
        }
        else if(i==fleet.numofship){
	...
	public void addeq(){
        ...
        switch (i)
        {
        case 1:cash-=fleet.addweap();break;
        case 2:fleet.unload();break;
        default:break;
        }
        }
	}
